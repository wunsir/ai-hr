...
export function EmployeeSupplement({ candidate }: EmployeeSupplementProps) {
  const supplement = candidate.employeeSupplement;
  ...
  return (
    <section className="panel employee-supplement">
      <div className="panel-header">
        <div>
          <span className="panel-kicker">员工补充输入</span>
          <h3>员工补充 → 结构化待确认线索</h3>
        </div>
          ...
          {hasGenerated ? (
            <>
              <div className="evidence-title-row">
                {supplement.structured.dimension.map((dimension) => (
